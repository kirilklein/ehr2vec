...
class TestBatches(unittest.TestCase):
    @patch('ehr2vec.data.batch.load_exclude_pids', return_value=['100'])
    @patch('ehr2vec.data.batch.load_assigned_pids', return_value={'pretrain': [str(i) for i in range(10)], 'finetune': [str(i) for i in range(90, 100)]})
    def setUp(self, *args):
        self.cfg = ConfigMock()
...
class TestBatchTokenize(unittest.TestCase):
    ...
    @patch('ehr2vec.data.batch.BatchTokenize.load_and_filter_batch', return_value=({
    ...
    @patch('ehr2vec.data.batch.BatchTokenize.load_and_filter_batch', side_effect=[
    ...
    @patch('torch.save', return_value=None)
    @patch('ehr2vec.data.batch.BatchTokenize.save_tokenized_data', return_value=None)
