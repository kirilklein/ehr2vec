...
def insert_shap_values(all_shap_values:Dict[str, torch.Tensor],
    ...
    for feature, shap_value in shap_values.items():
        if feature=='concept':
            ...
            assert shap_value.shape[0] == attention_mask.shape[0], "Shap value and attention mask must have the same batch size."
...
def split_batch_into_bg_and_fg(embeddings:Dict[str, torch.Tensor], n_bg_samples=10):
    ...
    if n_bg_samples > len(embeddings['concept']):
        raise ValueError("Number of background samples must be smaller than the batch size.")
