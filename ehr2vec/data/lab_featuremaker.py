...
class FeatureMaker:
    ...
    def create_features(self, concepts: pd.DataFrame) -> Tuple[dict, list]:
        # Add standard info
        pids = []
        if 'quartile_value' in self.features:
            del self.features['quartile_value']
        if 'binned_value' in self.features:
            del self.features['binned_value']
        for pid, patient in concepts.groupby('PID', sort=False):
            pids.append(pid)
            for feature, value in self.features.items():
                value.append(patient[feature.upper()].tolist())

        print(self.features['concept'][0])
